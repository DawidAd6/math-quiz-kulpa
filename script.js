...
function buildSessionQuestions() {
  const topic = topicSelectEl.value || "all";
...
function finishQuestion(isCorrect, question) {
  ...
  setTimeout(() => {
    if (currentQuestion >= sessionQuestions.length || sessionQuestions.length === 0) {
      showSummary();
    } else {
      currentQuestion++;
      showQuestion();
...
function showQuestion() {
  ...
  if (!question) {
    // brak pytań dla wybranego topicu
    questionTextEl.innerHTML = "Brak pytań dla wybranego tematu.";
    answersEl.innerHTML = "";
    return;
  }
...
function showSummary() {
  ...
  const count = sessionQuestions.length || 1;
  const avgTime = totalTime / count;
  summaryCountEl.textContent = `Poprawne odpowiedzi: ${correctCount} / ${count}`;
